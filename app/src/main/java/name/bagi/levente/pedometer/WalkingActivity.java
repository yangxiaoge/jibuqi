...
public class WalkingActivity extends Activity {
	...
	private float mDesiredPaceOrSpeed;// mDesiredPaceOrSpeed的值
	...
	public void onCreate(Bundle savedInstanceState) {
		Log.i(TAG, "[ACTIVITY] onCreate"); // 在Eclipse控制台查看日志
		super.onCreate(savedInstanceState);

		mStepValue = 0;
		mPaceValue = 0;

		setContentView(R.layout.jibu); // 加载计步器的布局文件
	...
	private void startStepService() {
		if (!mIsRunning) {
			...
			mIsRunning = true;
			startService(new Intent(WalkingActivity.this, StepService.class));
		}
	}

	private void bindStepService() {
		Log.i(TAG, "[SERVICE] Bind");
		bindService(new Intent(WalkingActivity.this, StepService.class),
	...
	private void resetValues(boolean updateDisplay) {
		if (mService != null && mIsRunning) {//true的时候
			mService.resetValues();
		} else {//false的时候
			mStepValueView.setText("0");//为对应文本框赋值为0
			...
			if (updateDisplay) {
				stateEditor.putInt("steps", 0);//将数据保存在shared_prefs目录下state.xml文件中
	...
	/* Creates the menu items 设置菜单项 */
	public boolean onPrepareOptionsMenu(Menu menu) {
		menu.clear();
		if (mIsRunning) {
			menu.add(0, MENU_PAUSE, 0, R.string.pause)//暂停
					.setIcon(android.R.drawable.ic_media_pause)
					.setShortcut('1', 'p');
		} else {
			menu.add(0, MENU_RESUME, 0, R.string.resume)//开始
					.setIcon(android.R.drawable.ic_media_play)
					.setShortcut('1', 'p');
		}
		menu.add(0, MENU_RESET, 0, R.string.reset)//重置
				.setIcon(android.R.drawable.ic_menu_close_clear_cancel)
				.setShortcut('2', 'r');
		menu.add(0, MENU_SETTINGS, 0, R.string.settings)
				.setIcon(android.R.drawable.ic_menu_preferences)
				.setShortcut('8', 's')
				.setIntent(new Intent(this, Settings.class)); //点击设置跳转并加载基本设置
		menu.add(0, MENU_QUIT, 0, R.string.quit)//退出
				.setIcon(android.R.drawable.ic_lock_power_off)
				.setShortcut('9', 'q');
		return true;
	}

	/* Handles item selections 处理点击菜单项事件*/
